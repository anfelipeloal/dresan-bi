...
async function handleSubmit({ data }) {
    try {
        ...
        const dataSet = await corporateProvider.getTravelSubType(data);
        ...
        const travelTypes = dataSet.map((test) => test.service_sub_type);

        dataOthers.primaryTravelTypes = travelTypes.join(',');

        const otherDataSet = await corporateProvider.getTravelSubTypeOther(dataOthers);
